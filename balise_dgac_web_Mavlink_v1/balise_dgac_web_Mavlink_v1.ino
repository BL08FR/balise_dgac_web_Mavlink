...
SoftwareSerial _MavLinkSerial(0, 2); // PIN TX & RX /!\What's written on the board don't necessary match GPIO pins! Using pin 0/2 don't allow to upload a sketch, you must disconnect before.
...
unsigned int lastHB;
unsigned long timetab[] = {0, 0, 0, 0, 0};// 0 is first Unix time sync memory, 1 is millis() when first UNIX time was set, 2 is actual Unix time, 3 mavlink restart time, 4 last home request sent
byte nbsat;
float hdopdata;
int homealt = 0;
float gpsRawAlt = 0;
        ...
        case MAVLINK_MSG_ID_GLOBAL_POSITION_INT:  // #33
          {
            ...
            if(has_home_set == 0 && nbsat >= 5 && hdopdata <5){
        ...
        case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW: // #36: Servo output raw
          {
            ...
            if(packet.servo9_raw > 1300 && packet.servo9_raw < 1700){Serial.println("Servo rail photo trigger PWM detected !");}
          }
          break;
        ...
        case MAVLINK_MSG_ID_RC_CHANNELS: // #65: RC channels
          {
            ...
            if(packet.chan9_raw > 1300 && packet.chan9_raw < 1700){Serial.println("RC Photo PWM detected !");}
            if(packet.chan9_raw >= 1700 && packet.chan9_raw < 2100){Serial.println("RC Video PWM detected !");}
          }
          break;
